...
Direct bamboohr — native REST surface (OAuth2 token + Employee) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { bamboohrPlugin } from "@emulators/bamboohr";

const { app } = createServer(bamboohrPlugin, { baseUrl: "http://localhost:4000" });
```
