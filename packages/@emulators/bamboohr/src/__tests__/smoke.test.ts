...
import { bamboohrPlugin, seedFromConfig, storeToSeedConfig } from "../index.js";
...
function mk() {
  ...
  const app = new Hono();
  bamboohrPlugin.register(app as never, store, new WebhookDispatcher(), base);
  return { app, store };
}
...
describe("bamboohr standalone native (native-kit, seed-derived)", () => {
  for (const model of spec.models) {
    it(`serves seeded ${model.model} at ${model.collectionPath}`, async () => {
      const { app, store } = mk();
      bamboohrPlugin.seed?.(store, base);
