...
Direct mixpanel — native REST surface (OAuth2 token + Event) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { mixpanelPlugin } from "@emulators/mixpanel";

const { app } = createServer(mixpanelPlugin, { baseUrl: "http://localhost:4000" });
```
