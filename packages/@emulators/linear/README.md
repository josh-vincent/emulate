...
Direct linear — native REST surface (OAuth2 token + Issue) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { linearPlugin } from "@emulators/linear";

const { app } = createServer(linearPlugin, { baseUrl: "http://localhost:4000" });
```
