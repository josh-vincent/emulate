...
Direct zoho-crm — native REST surface (OAuth2 token + Leads) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { zohoCrmPlugin } from "@emulators/zoho-crm";

const { app } = createServer(zohoCrmPlugin, { baseUrl: "http://localhost:4000" });
```
