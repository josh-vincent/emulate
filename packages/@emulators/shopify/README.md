...
Direct shopify — native REST surface (OAuth2 token + Product, Order) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { shopifyPlugin } from "@emulators/shopify";

const { app } = createServer(shopifyPlugin, { baseUrl: "http://localhost:4000" });
```
