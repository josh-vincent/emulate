...
Direct clickup — native REST surface (OAuth2 token + Task) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { clickupPlugin } from "@emulators/clickup";

const { app } = createServer(clickupPlugin, { baseUrl: "http://localhost:4000" });
```
