...
Direct monday — native REST surface (OAuth2 token + Item) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { mondayPlugin } from "@emulators/monday";

const { app } = createServer(mondayPlugin, { baseUrl: "http://localhost:4000" });
```
