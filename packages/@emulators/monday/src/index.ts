...
// Standalone direct-to-source monday emulator. Mount it and clients hit
// monday's real native paths (/items) behind a bearer token — no Nango envelope. The Nango
// emulator remains the alternative; this is the "go direct" option.

const built = makeNativePlugin(spec);

export const mondayPlugin = built.plugin;
