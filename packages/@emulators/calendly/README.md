...
Direct calendly — native REST surface (OAuth2 token + ScheduledEvent) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { calendlyPlugin } from "@emulators/calendly";

const { app } = createServer(calendlyPlugin, { baseUrl: "http://localhost:4000" });
```
