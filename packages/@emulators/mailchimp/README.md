...
Direct mailchimp — native REST surface (OAuth2 token + Member, Campaign) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { mailchimpPlugin } from "@emulators/mailchimp";

const { app } = createServer(mailchimpPlugin, { baseUrl: "http://localhost:4000" });
```
