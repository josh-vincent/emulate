...
Direct trello — native REST surface (OAuth2 token + Card, Board) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { trelloPlugin } from "@emulators/trello";

const { app } = createServer(trelloPlugin, { baseUrl: "http://localhost:4000" });
```
