...
Direct lever — native REST surface (OAuth2 token + Opportunity) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { leverPlugin } from "@emulators/lever";

const { app } = createServer(leverPlugin, { baseUrl: "http://localhost:4000" });
```
