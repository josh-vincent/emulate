...
Direct pipedrive — native REST surface (OAuth2 token + Person, Deal) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { pipedrivePlugin } from "@emulators/pipedrive";

const { app } = createServer(pipedrivePlugin, { baseUrl: "http://localhost:4000" });
```
