...
Direct dropbox — native REST surface (OAuth2 token + File) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { dropboxPlugin } from "@emulators/dropbox";

const { app } = createServer(dropboxPlugin, { baseUrl: "http://localhost:4000" });
```
