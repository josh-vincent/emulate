...
Direct discord — native REST surface (OAuth2 token + Channel, Message) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { discordPlugin } from "@emulators/discord";

const { app } = createServer(discordPlugin, { baseUrl: "http://localhost:4000" });
```
