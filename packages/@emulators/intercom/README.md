...
Direct intercom — native REST surface (OAuth2 token + Contact, Conversation) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { intercomPlugin } from "@emulators/intercom";

const { app } = createServer(intercomPlugin, { baseUrl: "http://localhost:4000" });
```
