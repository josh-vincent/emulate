...
Direct klaviyo — native REST surface (OAuth2 token + Profile) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { klaviyoPlugin } from "@emulators/klaviyo";

const { app } = createServer(klaviyoPlugin, { baseUrl: "http://localhost:4000" });
```
