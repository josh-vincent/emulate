...
Direct sendgrid — native REST surface (OAuth2 token + Contact) driven by @emulators/native-kit from the SDK-aligned seed.
...
import { sendgridPlugin } from "@emulators/sendgrid";

const { app } = createServer(sendgridPlugin, { baseUrl: "http://localhost:4000" });
```
