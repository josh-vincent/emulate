...
describe("Simpro sections + cost centers", () => {
  it("lists sections with display_order", async () => {
    ...
    const res = await app.request(
      `${BASE}/api/v1.0/companies/0/jobs/12345/sections/`,
      { headers: auth(token) },
    );
  ...
  it("lists section-nested cost centers with display=all expands Items", async () => {
    ...
    const token = await getAccessToken(app);
    const res = await app.request(
      `${BASE}/api/v1.0/companies/0/jobs/12345/sections/1001/costCenters/?display=all`,
      { headers: auth(token) },
    );
    expect(res.status).toBe(200);
  ...
  it("top-level /jobs/:jid/costCenters/ returns cross-section list", async () => {
    ...
    const token = await getAccessToken(app);
    const res = await app.request(
      `${BASE}/api/v1.0/companies/0/jobs/12345/costCenters/`,
      { headers: auth(token) },
    );
  ...
  it("POST section then POST cost center", async () => {
    ...
    const ccRes = await app.request(
      `${BASE}/api/v1.0/companies/0/jobs/12345/sections/${section.ID}/costCenters/`,
      {
        method: "POST",
        headers: auth(token),
        body: JSON.stringify({
  ...
  it("master cost centers listed at /setup/costCenters/", async () => {
    ...
    const token = await getAccessToken(app);
    const res = await app.request(
      `${BASE}/api/v1.0/companies/0/setup/costCenters/`,
      { headers: auth(token) },
    );
  ...
  it("DELETE section cascades to its cost centers", async () => {
    ...
    const crossSection = await app.request(
      `${BASE}/api/v1.0/companies/0/jobs/12345/costCenters/`,
      { headers: auth(token) },
    );
