...
//   POST /oauth/authorize/callback → mints code, redirects
//   POST /oauth/v1/token           → exchanges code for tokens
//   GET  /oauth/v1/access-tokens/:token → identity introspection
//
...
export const directHubspotRoutes = (app: Hono<AppEnv>, store: Store): void => {
  ...
  app.post("/oauth/v1/token", async (c) => {
